...
verify_native_bundle() {
  ...
  grep -q 'name="ios-native-build" content="80"' www/index.html || fail "Generated www bundle is not native build 80."
  ...
  grep -q 'id="c64BootOverlay"' www/index.html || fail "Native C64 boot overlay was not packaged."
  grep -q 'id="c64BootCanvas"' www/index.html || fail "Native PETSCII boot canvas was not packaged."
  grep -q 'c64-boot-screen::before' www/index.html || fail "Native boot CRT scanlines were not packaged."
  grep -q 'ios-boot.js?v=80' www/index.html || fail "Native C64 boot script is not linked."
  grep -q 'ROM_B64' www/ios-boot.js || fail "Native boot is not using the C64 character ROM."
  grep -q 'centerColumn' www/ios-boot.js || fail "Native C64 boot header centering is missing."
  grep -q 'COMMODORE 64 BASIC V2' www/ios-boot.js || fail "Native C64 boot sequence is missing BASIC startup text."
  grep -q 'LOAD"\*",8,1' www/ios-boot.js || fail "Native C64 disk-loading sequence is missing."
  grep -q 'ios-focus.js?v=80' www/index.html || fail "Native focus walk-in script is not linked."
  ...
  grep -q 'name="ios-native-build" content="80"' "$PUBLIC_INDEX" || fail "Xcode public bundle is stale; expected native build 80."
  ...
  grep -q 'id="c64BootCanvas"' "$PUBLIC_INDEX" || fail "Xcode public bundle is missing PETSCII boot canvas."
  grep -q 'c64-boot-screen::before' "$PUBLIC_INDEX" || fail "Xcode public bundle is missing boot CRT scanlines."
  grep -q 'ROM_B64' "$PUBLIC_BOOT" || fail "Xcode public bundle is missing C64 ROM PETSCII boot behavior."
  grep -q 'centerColumn' "$PUBLIC_BOOT" || fail "Xcode public bundle is missing centered C64 header behavior."
  ...
  say "Native bundle verification: OK (build 80)"
}
